    ...
    if (variant === "LOGIN") {
      signIn("credentials", {
        ...
        .then((res) => {
          ...
          if (res?.ok && !res?.error) {
            toast.success("Entering Chat Room!");
            router.push("/users");
          }
        })
        .finally(() => setIsLoading(false));
    ...
    <section className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
      <div className="bg-white px-4 py-8 shadow sm:rounded-lg sm:px-10">
        ...
        <div className="flex gap-2 justify-center text-sm mt-6 px-2 text-gray-500">
          <span>{variant === "REGISTER" ? "Already have an account?" : "New to Our Chat Room?"}</span>
          <button type="button" onClick={toggleVariant} className="underline text-blue-500 hover:text-blue-600">
